...
ssh rpuzak@192.168.31.248 "cd ~/ohmatdyt_crm && git pull origin main"

# Step 3: Rebuild and restart frontend container
Write-Host "`n[3/4] Rebuilding frontend container..." -ForegroundColor Yellow
ssh rpuzak@192.168.31.248 "cd ~/ohmatdyt_crm/ohmatdyt-crm && docker compose -f docker-compose.yml -f docker-compose.prod.yml build frontend"

Write-Host "`n[4/4] Restarting frontend container..." -ForegroundColor Yellow
ssh rpuzak@192.168.31.248 "cd ~/ohmatdyt_crm/ohmatdyt-crm && docker compose -f docker-compose.yml -f docker-compose.prod.yml up -d frontend"

# Step 4: Verify
Write-Host "`n=== Verification ===" -ForegroundColor Cyan
Write-Host "Check the following:" -ForegroundColor Green
Write-Host "1. Frontend logs: ssh rpuzak@192.168.31.248 'cd ~/ohmatdyt_crm/ohmatdyt-crm && docker compose logs frontend | tail -20'" -ForegroundColor White
Write-Host "2. Try login at: https://192.168.31.248/login" -ForegroundColor White
Write-Host "3. Check browser console for API requests (should go to http://192.168.31.248/api)" -ForegroundColor White
